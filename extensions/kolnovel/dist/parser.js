...
function parseInlineStyle(styleAttr) {
    ...
    for (const rule of styleAttr.split(";")) {
        ...
        if (colonIdx === -1)
            continue;
        const key = rule.slice(0, colonIdx).trim().replace(/-([a-z])/g, (_, c) => c.toUpperCase());
 ...
 */
function isHiddenParagraph(p) {
    const styleAttr = p.getAttribute("style") ?? "";
    if (!styleAttr)
        return false;
    const s = parseInlineStyle(styleAttr);
    return (s["height"] === "0.1px" &&
        s["overflow"] === "hidden" &&
        s["position"] === "fixed" &&
        s["opacity"] === "0" &&
 ...
 */
function isSpamLine(text) {
    const lower = text.toLowerCase();
    return (lower.includes("kolnovel") ||
...
export function parseHomePage(html) {
    ...
    doc.querySelectorAll(".slide-item").forEach((item) => {
        const linkEl = item.querySelector(".slide-content a[href]");
        const titleEl = item.querySelector(".slide-content .title .ellipsis a");
        const coverEl = item.querySelector("img[src]");
        if (!linkEl)
            return;
        const href = linkEl.getAttribute("href") ?? "";
        if (!href)
            return;
        const latestChapter = item.querySelector(".slidchap")?.textContent?.trim() ?? undefined;
        const statusRaw = item.querySelector(".cast .director")?.textContent?.trim() ?? "";
        const ratingRaw = item.querySelector(".rt .vote span span")?.textContent?.trim() ?? "";
    ...
    doc.querySelectorAll(".hotoday .inhotoday a.tip").forEach((el) => {
        ...
        if (!href)
            return;
        const latestChapter = anchor.querySelector(".todchap")?.textContent?.trim() ?? undefined;
        const statusRaw = anchor.querySelector(".todstat")?.textContent?.trim() ?? "";
        const ratingRaw = anchor.querySelector(".todsco .todnum")?.textContent?.trim() ?? "";
        const rating = Number.parseFloat(ratingRaw);
        const genres = [];
    ...
    doc.querySelectorAll(".utao .uta").forEach((uta) => {
        const linkEl = uta.querySelector(".imgu a");
        const titleEl = uta.querySelector(".luf h3");
        const imgEl = uta.querySelector("img");
        if (!linkEl)
            return;
        const href = linkEl.getAttribute("href") ?? "";
        const latestChapter = uta.querySelector(".luf ul li a")?.textContent?.trim() ?? undefined;
        latestUpdates.push({
...
export function parseBrowsePage(html) {
    ...
    doc.querySelectorAll(".bsx").forEach((item) => {
        ...
        if (!href)
            return;
        const latestChapter = item.querySelector(".epxs")?.textContent?.trim() ?? undefined;
        results.push({
            title: titleEl?.textContent?.trim() ?? "",
            url: absoluteUrl(href),
...
export function parseNovelDetail(html, novelUrl) {
    ...
    // Rating
    const ratingText = doc.querySelector(".rating .num")?.textContent?.trim() ??
        doc.querySelector(".rt .num")?.textContent?.trim() ?? "";
    ...
    if (volumeToggles.length === 0) {
        ...
        doc.querySelectorAll(".eplister ul li a").forEach((a) => {
            const anchor = a;
            const href = anchor.getAttribute("href") ?? "";
            const chapterTitle = anchor.querySelector(".epl-title")?.textContent?.trim() ??
        ...
        volumeToggles.forEach((toggle, idx) => {
            const volTitle = toggle.querySelector("button span, button")?.textContent?.trim() ??
                `Volume ${idx + 1}`;
...
// ---------------------------------------------------------------------------
export function parseChapterContent(html, _chapterUrl) {
    const doc = parseHtml(html);
    const title = doc.querySelector(".entry-title")?.textContent?.trim() ??
